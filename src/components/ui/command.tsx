...
function CommandItem({
  ...
  /** Secondary action fired on ⌘↵ instead of onSelect */
  onMetaSelect?: () => void
}) {
  const handleSelect: typeof onSelect = onMetaSelect
    ? (...args) => {
  ...
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "group/command-item relative flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none in-data-[slot=dialog-content]:rounded-lg! data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 data-selected:bg-muted data-selected:text-foreground [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 data-selected:*:[svg]:text-foreground transition-[background-color] duration-75 delay-[25ms]",
        className,
      )}
